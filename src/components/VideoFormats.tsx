...
const VideoFormats: React.FC<VideoFormatsProps> = ({ data }) => {
  return (
    <div className="w-full">
      <div className="text-[1.8rem] my-[1rem] font-semibold">Raw Formats!</div>

      <div className="pb-4 text-[1.8rem]">
      ...
      <Table
        headings={["Quality", "Has Audio", "Actions"]}
        className="min-w-full"
        data={data.formats
          ?.filter((i) => i?.hasAudio && i?.hasVideo)
          ?.map((format) => ({
            quality:
            ...
            url: (
              <>
                <a
                  className="text-[1.6rem] py-[0.7rem] px-[1rem] bg-[#6AC5F6] text-white rounded-lg"
                  href={format.url}
                  target="_blank"
                  rel="noreferrer"
                >
